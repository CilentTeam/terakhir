...
const { xnxxsearch, xnxxdl } = require('./scrape/xnxx')
...
const { smsg, formatp, formatDate, getTime, isUrl, sleep, clockString, runtime, fetchJson, getBuffer, jsonformat, format, parseMention, getRandom, reSize } = require('./Lib/myfunc')
const { tanggal } = require('./Lib/myfunc')
...
try {
    ...
    const ddetik = Math.floor( Selisih % (1000 * 60) / 1000)
        ...
        const banUser = await Pares.fetchBlocklist()
...
img: await resz.getBufferAsync(jimp.MIME_JPEG)
        ...
        const sendFileFromUrl = async (from, url, caption, m, men) => {
            ...
            if (mime.split("/")[1] === "gif") {
                return Pares.sendMessage(from, { video: await getBuffer(url), caption: caption, gifPlayback: true, mentions: men ? men : [], mimetype: 'video/mp4'}, {quoted: m})
                }
            let type = mime.split("/")[0]+"Message"
            if(mime === "application/pdf"){
                return Pares.sendMessage(m.chat, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, mentions: men ? men : []}, {quoted: m })
            }
            if(mime.split("/")[0] === "image"){
                return Pares.sendMessage(m.chat, { image: await getBuffer(url), caption: caption, mentions: men ? men : []}, {quoted: m})
            }
            if(mime.split("/")[0] === "video"){
                return Pares.sendMessage(m.chat, { video: await getBuffer(url), caption: caption, mentions: men ? men : [], mimetype: 'video/mp4'}, {quoted: m })
            }
            if(mime.split("/")[0] === "audio"){
                return Pares.sendMessage(m.chat, { audio: await getBuffer(url), caption: caption, mentions: men ? men : [], mimetype: 'audio/mpeg'}, {quoted: m })
            }
        }
        ...
        Pares.sendMessage(m.chat, { text : teks, mentions: await Pares.parseMention(teks), contextInfo : {
...
"jpegThumbnail": ppnyauser
        ...
        const sticWait = (hehe) => {
			...
			Pares.sendMessage(hehe, { sticker: ano, contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
"previewType": "VIDEO",
"mediaUrl": 'https://youtu.be/b0CCKgVEhlk',
"sourceUrl": 'https://Pares-api.herokuapp.com'}}}, 
			{ quoted: floc })
		}
		const replyAlfa = (teks) => {
		Pares.sendMessage(m.chat, { text: teks, contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
"sourceUrl": 'https://Pares-api.herokuapp.com'}}}, 
        ...
        if (tebaklagu.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak lagu', buttonText: { displayText: 'Tebak Lagu' }, type: 1 }], `🎮 Tebak Lagu 🎮\n\nJawaban Benar 🎉\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
        ...
        if (tebakgambar.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak gambar', buttonText: { displayText: 'Tebak Gambar' }, type: 1 }], `🎮 Tebak Gambar 🎮\n\nJawaban Benar 🎉\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
        ...
        if (tebakkata.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak kata', buttonText: { displayText: 'Tebak Kata' }, type: 1 }], `🎮 Tebak Kata 🎮\n\nJawaban Benar 🎉\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
	    ...
	    deskripsi = caklontong_desk[m.sender.split('@')[0]]
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak lontong', buttonText: { displayText: 'Tebak Lontong' }, type: 1 }], `🎮 Cak Lontong 🎮\n\nJawaban Benar 🎉\n*${deskripsi}*\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
        ...
        if (tebakkalimat.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak kalimat', buttonText: { displayText: 'Tebak Kalimat' }, type: 1 }], `🎮 Tebak Kalimat 🎮\n\nJawaban Benar 🎉\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
        ...
        if (tebaklirik.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak lirik', buttonText: { displayText: 'Tebak Lirik' }, type: 1 }], `🎮 Tebak Lirik 🎮\n\nJawaban Benar 🎉\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
	...
	if (tebaktebakan.hasOwnProperty(m.sender.split('@')[0]) && isCmd) {
            ...
            if (budy.toLowerCase() == jawaban) {
                await Pares.sendButtonText(m.chat, [{ buttonId: 'tebak tebakan', buttonText: { displayText: 'Tebak Tebakan' }, type: 1 }], `🎮 Tebak Tebakan 🎮\n\nJawaban Benar 🎉\n\nIngin bermain lagi? tekan button dibawah`, Pares.user.name, m)
                delete tebaktebakan[m.sender.split('@')[0]]
            } else reply('*Jawaban Salah!*')
        }

_sewa.expiredCheck(Pares, sewa)
...
Pares.relayMessage(jid, order.message, { messageId: order.key.id})
}
...
async function ayatKursi() {
   return new Promise( async (resolve, reject) => {
       ...
       resolve(result)
        ...
        Pares.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
	    Pares.groupParticipantsUpdate(m.chat, [sender], 'remove')
		}
	    }
	    
	    if (m.isGroup && !m.key.fromMe &&  db.data.chats[m.chat].antiwame && !isGroupAdmins){
        if (budy.match(`wa.me`)) {
        Pares.sendMessage(m.chat, {text: `*Antiwame Group Terdeteksi*\n\nKamu Akan Dikeluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
	    Pares.groupParticipantsUpdate(m.chat, [sender], 'remove')
		}
	    }
        ...
        Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: await reSize(data.thumb, 200, 200) }, templateButtons: button, footer: `Pilih Salah Satu Button Dibawah⬇️`, mentions: [sender] })
        }).catch((e) => {
        Pares.sendMessage(from, { text: mess.error }, { quoted: m })
        ownerNumber.map( i => Pares.sendMessage(from, { text: `Send Play Error : ${e}` }))
        })
        }
...
🛒 𝗦𝗲𝗹𝗹 𝗜𝗻𝘃𝗲𝗻𝘁𝗼𝗿𝘆
 ...
 × ${prefix}sellemas
 × ${prefix}selldiamond`
        Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: await reSize(thumbrpg, 200, 200) }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
「 PROFILE 」
 ...
 • Xp :  ${getLevelingXp(sender)}/${reqXp}
「 RPG GAME 」`
        Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgjoinrpg }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
addDm(sender, d)
        ...
        Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgslime }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imggoblin }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdevil }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgbehemoth }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdemon }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: imgdemonking }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
...
async function replyTemplate(teks) {
                       ...
                       return Pares.sendMessage(from, buttonMessage)
                }
...
Pares.sendMessage(from, { caption: teks, location: { jpegThumbnail: thumb }, templateButtons: button, footer: `Owner Bot : ${owner.length}`, mentions: [sender] })
        .catch ((err) => reply(err))
        }
        const sendMenu = (teks) => {
        Pares.sendMessage(m.chat, { text: teks }, { quoted: m })
        }
...
switch (command) {
case 'xnxxdl': {
if (!q) throw 'Contoh: xnxxdl https://www.xnxx.com/video-z1hol38/jav_cute_babe_in_uniform_enjoys_double_penetration'
...
break
case 'bot': {
replyAlfa(`On Kok Kk ${pushname}`)
}
break
case 'xnxxsearch': {
if (!q) throw 'Contoh: xnxxsearch Japanese'
const data = await xnxxsearch(q).catch(e => {
      m.reply("eror")
      })
      console.log(data)
...
const sendMsg = await Pares.sendMessage(m.chat, listMessage)
break
case prefix+'addlist':
if (!isCreator) return reply(`Kmu Bukan Owner Kk ${pushname}`)
...
https://chat.whatsapp.com/${response}
       `
Pares.sendMessage(m.chat, { text:teks, mentions: await Pares.parseMention(teks)}, {quoted:m})
} catch {
reply("Error!")
}
}
...
try {
reply(`// Created By Pares\n` + 'case ' + `'${args[0]}'` + fs.readFileSync('./Pares.js').toString().split(`case '${args[0]}'`)[1].split(turbrek)[0] + turbrek)
} catch {
...
Pares.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Fitur Nulis By Pares ", fejfjej, { quoted: m })
break
case 'nuliskiri':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
.on('exit', () => {
Pares.sendMessage(from, { image: fs.readFileSync('./Kir/FOTO/BUKU/setelahkiri.jpg'), contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
"previewType": "VIDEO",
"mediaUrl": 'https://youtu.be/b0CCKgVEhlk',
"sourceUrl": 'https://Pares-api.herokuapp.com'}}}, {quoted: m, caption: `Jangan Malas Kak...`})
})
}
break
case 'nuliskanan':{
...
replyNya(mess.wait)
                ...
                .on('error', () => reply('error'))
                .on('exit', () => {
                Pares.sendMessage(from, { image: fs.readFileSync('./Kir/FOTO/BUKU/setelahkanan.jpg'), contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
replyNya(mess.wait)
                ...
                .on('error', () => reply('error'))
                .on('exit', () => {
                Pares.sendMessage(from, { image: fs.readFileSync('./Kir/FOTO/FOLIO/setelahkiri.jpg'), contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
replyNya(mess.wait)
                ...
                .on('exit', () => {
                Pares.sendMessage(from, { image: fs.readFileSync('./Kir/FOTO/FOLIO/setelahkanan.jpg'), contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
Pares.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Topup Game By Pares ", secmmknya, { quoted: efff })
break
case 'topup5':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
if (!args[0]) return replyNya(`Example : \n${prefix + command} 946716486`)
...
let message = await prepareWAMessageMedia({ image: fs.readFileSync('./Image/gopaykir.jpg') }, { upload: Pares.waUploadToServer })
...
Pares.relayMessage(m.chat, template.message, { messageId: template.key.id })
}
break
case 'topup12':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
const isblocks = await Pares.fetchBlocklist()
const isBlock = isblocks.includes(jidny)
texnr = `     「 Profile Inspector 」
▸ Number : ${jidny.split("@")[0]}
▸ Mention : @${jidny.split("@")[0]}
▸ Name : ${Pares.getName(jidny)}
▸ Biography : ${jsonformat(await Pares.fetchStatus(jidny).catch(() => {}))}
▸ Business : ${jsonformat(await Pares.getBusinessProfile(jidny))}
...
▸ Blockir : ${isBlock}`
try {
ppuser = await Pares.profilePictureUrl(jidny, 'image')
} catch {
ppuser = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
Pares.sendMessage(m.chat, { image: { url: ppuser }, caption: texnr, mentions: await Pares.parseMention(texnr), contextInfo:{externalAdReply:{
...
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
                ...
                if (args[0] === "lagu") {
                    ...
                    console.log(result)
                    let msg = await Pares.sendMessage(m.chat, { audio: { url: result.link_song }, mimetype: 'audio/mpeg' }, { quoted: m })
                    Pares.sendText(m.chat, `Judul Lagu Nya Apa??\n\nArtist : ${result.artist}\nWaktu : 60s`, msg).then(() => {
                    tebaklagu[m.sender.split('@')[0]] = result.jawaban.toLowerCase()
                    })
                    await sleep(60000)
                    if (tebaklagu.hasOwnProperty(m.sender.split('@')[0])) {
                    console.log("Jawaban: " + result.jawaban)
                    Pares.sendButtonText(m.chat, [{ buttonId: 'tebak lagu', buttonText: { displayText: 'Tebak Lagu' }, type: 1 }], `Waktu Habis\nJawaban:  ${tebaklagu[m.sender.split('@')[0]]}\n\nIngin bermain? tekan button dibawah`, Pares.user.name, m)
                    delete tebaklagu[m.sender.split('@')[0]]
                    ...
                    let result = anu[Math.floor(Math.random() * anu.length)]
                    console.log(result)
                    Pares.sendImage(m.chat, result.img, `Silahkan Jawab Soal Di Atas Ini\n\nDeskripsi : ${result.deskripsi}\nWaktu : 60s`, m).then(() => {
                    tebakgambar[m.sender.split('@')[0]] = result.jawaban.toLowerCase()
                    })
                    await sleep(60000)
                    if (tebakgambar.hasOwnProperty(m.sender.split('@')[0])) {
                    console.log("Jawaban: " + result.jawaban)
                    Pares.sendButtonText(m.chat, [{ buttonId: 'tebak gambar', buttonText: { displayText: 'Tebak Gambar' }, type: 1 }], `Waktu Habis\nJawaban:  ${tebakgambar[m.sender.split('@')[0]]}\n\nIngin bermain? tekan button dibawah`, Pares.user.name, m)
                    delete tebakgambar[m.sender.split('@')[0]]
                    ...
                    let result = anu[Math.floor(Math.random() * anu.length)]
                    console.log(result)
                    Pares.sendText(m.chat, `Silahkan Jawab Pertanyaan Berikut\n\n${result.soal}\nWaktu : 60s`, m).then(() => {
                    tebakkata[m.sender.split('@')[0]] = result.jawaban.toLowerCase()
                    })
                    await sleep(60000)
                    if (tebakkata.hasOwnProperty(m.sender.split('@')[0])) {
                    console.log("Jawaban: " + result.jawaban)
                    Pares.sendButtonText(m.chat, [{ buttonId: 'tebak kata', buttonText: { displayText: 'Tebak Kata' }, type: 1 }], `Waktu Habis\nJawaban:  ${tebakkata[m.sender.split('@')[0]]}\n\nIngin bermain? tekan button dibawah`, Pares.user.name, m)
                    delete tebakkata[m.sender.split('@')[0]]
                    ...
                    console.log("Jawaban: " + result.jawaban)
                    Pares.sendButtonText(m.chat, [{ buttonId: 'tebak kalimat', buttonText: { displayText: 'Tebak Kalimat' }, type: 1 }], `Waktu Habis\nJawaban:  ${tebakkalimat[m.sender.split('@')[0]]}\n\nIngin bermain? tekan button dibawah`, Pares.user.name, m)
                    delete tebakkalimat[m.sender.split('@')[0]]
                    ...
                    let result = anu[Math.floor(Math.random() * anu.length)]
                    console.log(result)
                    Pares.sendText(m.chat, `Ini Adalah Lirik Dari Lagu? : *${result.soal}*?\nWaktu : 60s`, m).then(() => {
                    tebaklirik[m.sender.split('@')[0]] = result.jawaban.toLowerCase()
                    })
                    await sleep(60000)
                    if (tebaklirik.hasOwnProperty(m.sender.split('@')[0])) {
                    console.log("Jawaban: " + result.jawaban)
                    Pares.sendButtonText(m.chat, [{ buttonId: 'tebak lirik', buttonText: { displayText: 'Tebak Lirik' }, type: 1 }], `Waktu Habis\nJawaban:  ${tebaklirik[m.sender.split('@')[0]]}\n\nIngin bermain? tekan button dibawah`, Pares.user.name, m)
                    delete tebaklirik[m.sender.split('@')[0]]
                    ...
                    let result = anu[Math.floor(Math.random() * anu.length)]
                    console.log(result)
                    Pares.sendText(m.chat, `*Jawablah Pertanyaan Berikut :*\n${result.soal}*\nWaktu : 60s`, m).then(() => {
		    ...
		    caklontong_desk[m.sender.split('@')[0]] = result.deskripsi
                    ...
                    console.log("Jawaban: " + result.jawaban)
                    Pares.sendButtonText(m.chat, [{ buttonId: 'tebak lontong', buttonText: { displayText: 'Tebak Lontong' }, type: 1 }], `Waktu Habis\nJawaban:  ${caklontong[m.sender.split('@')[0]]}\nDeskripsi : ${caklontong_desk[m.sender.split('@')[0]]}\n\nIngin bermain? tekan button dibawah`, Pares.user.name, m)
                    delete caklontong[m.sender.split('@')[0]]
		    delete caklontong_desk[m.sender.split('@')[0]]
...
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
                ...
                Pares.sendText(m.chat, `*Berapa hasil dari: ${result.soal.toLowerCase()}*?\n\nWaktu: ${(result.waktu / 1000).toFixed(2)} detik`, m).then(() => {
...
Alfa.sendMessage(m.chat, { image: { url : aj.thumb }, caption: 
`*/ Search Lirik \\*
...
let message = await prepareWAMessageMedia({ image: hasil }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
hydratedContentText: `Gelap Bos:v`,
hydratedFooterText: 'Created By Pares',
hydratedButtons: [{
urlButton: {
displayText: 'MyGithub',
url: 'https://github.com/Pares'
}
},
{
urlButton: {
...
let message = await prepareWAMessageMedia({ image: memeall }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
let message = await prepareWAMessageMedia({ image: highschooldxd }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
let message = await prepareWAMessageMedia({ image: swordartonline }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
let message = await prepareWAMessageMedia({ image: lovelive }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
if (!m.key.fromMe && !isCreator) return replyNya(mess.owner)
teks = '*List Premium*\n\n'
for (let Pares of prem) {
teks += `- ${Pares}\n`
}
teks += `\n*Total : ${prem.length}*`
Pares.sendMessage(from, { text: teks.trim() }, 'extendedTextMessage', { quoted: m, contextInfo: { "mentionedJid": prem } })
break
case 'addprem':
if (!m.key.fromMe && !isCreator) return replyNya(mess.owner)
...
Pares.sendMessage(m.chat, {audio: inicdd, mimetype:'audio/mpeg', ptt:true }, {quoted:m})
break
...
Pares.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Rpg Game By Pares ", sectionnya, { quoted: fakePares})
break
case 'slime':
case 'killslime':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Saya Pares Saya Disini Ingin Membantu Kalian Membuat Sticker Download Video/Lagu Youtube, Tiktok Dll.`
let buttons = [
...
"previewType": "NONE",
"mediaUrl": 'https://Pares-api.herokuapp.com',
"sourceUrl": 'https://Pares-api.herokuapp.com'
}}
}
Pares.sendMessage(m.chat, buttonMessage, { quoted: {
				  key: {
				   fromMe: false,
				   participant: `0@s.whatsapp.net`,
				   remoteJid: "6281903153426-1626053991@g.us"
...
│ ${ya}${Yy+=1} < [EVAL2]${ya}
│ ${ya}${Yy+=1} => [ASYNC]${ya}
│ ${ya}${Yy+=1} ${prefix}alfadoc 9${ya}
│ ${ya}${Yy+=1} ${prefix}okehh 9${ya}
│ ${ya}${Yy+=1} ${prefix}ampas 9${ya}
...
│ ${ya}${Yy+=1} ${prefix}addprem 628×××${ya}
│ ${ya}${Yy+=1} ${prefix}delprem 628×××${ya}
│ ${ya}${Yy+=1} ${prefix}listpremium${ya}
│ ${ya}${Yy+=1} ${prefix}ban/block add 628×××${ya}
│ ${ya}${Yy+=1} ${prefix}ban/block del 628×××${ya}
...
│ ${ya}${Yy+=1} ${prefix}mlstalk <id|zone id>${ya}
...
Pares.send5Loc(m.chat, `On Kak Silahkan Pencet Button Di Bawah\nJika Button Tidak Kelihatan Ketik ${prefix}menu`, `Created By Pares`, global.thumb, btn)
}
break
case 'userPares':{
...
if (/webp/.test(mime)) {
Pares.sendMessage(i, { sticker: { url: media } }, { quoted: m })
} else if (/image/.test(mime)) {
let junn = `*_BROADCAST IMAGE_*${text ? '\n\n' + text : ''}`
Pares.send5ButImg(i, junn, `Created By Pares`, buffer, butoon)
} else if (/video/.test(mime)) {
let junn = `*_BROADCAST VIDIO_*${text ? '\n\n' + text : ''}`
Pares.send5Vid(i, junn, `Created By Pares`, buffer, butoon)
} else if (/audio/.test(mime)) {
Pares.sendMessage(i, {audio: buffer, mimetype: 'audio/mpeg'}, { quoted : m })
} else {
...
Pares.sendList(m.chat, `Haii *${pushname}*`, `Silahkan Pilih Di Bawah`, "CLICK", "Bug Button By Pares ", gejfjr, { quoted: lep })
}
...
ydd = `Hallo Aku Pares`
for (let i = 0; i < jumlah; i++) {
Pares.sendMessage(from, { document: await reSize(ppnyauser, 200, 200),  fileName: `${buttonvirus}`,  mimetype: `application/zip`, jpegThumbnail: await reSize(ppnyauser, 200, 200), fileLength: "999999999", mentions:[sender] }, {quoted:lep})
}
reply(`Sukses Send Bug Sebanyak ${jumlah}`)
}
break
...
waktu = q.split('|')[2]
for (let i = 0; i < jumlah; i++) {
var messa = await prepareWAMessageMedia({ image: ppnyauser }, { upload: Pares.waUploadToServer })
...
waktu = q.split('|')[2]
for (let i = 0; i < jumlah; i++) {
Pares.sendMessage(num, { text: 'Oii kimoyasaaa' }, { quoted: lep})
await sleep(toMs(waktu))
}
...
jumlah = q.split('|')[1]
for (let i = 0; i < jumlah; i++) {
var messa = await prepareWAMessageMedia({ image: thumb }, { upload: Pares.waUploadToServer })
var catalog = generateWAMessageFromContent(num, proto.Message.fromObject({
"productMessage": {
"product": {
"productImage": messa.imageMessage,
...
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
		...
		for (let yoi of anu) {
		    ...
		    let btn = [{
                      ...
                      Pares.send5ButImg(yoi, txt, Pares.user.name, global.thumb, btn)
		}
...
Pares.sendMessage(m.chat, { video: { url: hasiltod.result }, mimetype: 'video/mp4', fileName: `asupan.mp4`, caption: `Nih Asupan Nya` }, { quoted: fakePares })
}
break
case 'santuy':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, { video: { url: hasifs.result.url }, mimetype: 'video/mp4', fileName: `${command}.mp4`, caption: `Nih Asupan Nya` }, { quoted: fakePares })
}
break
case 'bokep':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, { video: dwhe, mimetype: 'video/mp4', fileName: `${command}.mp4`, caption: `Nih Bokep Nya` }, { quoted: fakePares })
}
break
case 'storyanime':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, { video: { url: hasiltod.result }, mimetype: 'video/mp4', fileName: `storyanime.mp4`, caption: `Nih Story Animenya` }, { quoted: fakePares })
}
break
case 'out':{
if (!isCreator) return replyNya(mess.owner)
Pares.groupLeave(from)
}
break
case 'public': {
if (!isCreator) return replyNya(mess.owner)
...
Pares.sendMessage(m.chat, { image: buffer, contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
"previewType": "VIDEO",
"mediaUrl": 'https://youtu.be/b0CCKgVEhlk',
"sourceUrl": 'https://Pares-api.herokuapp.com'}}}, { quoted: fakePares })
fs.unlinkSync(ran)
})
}
break
...
.then(({data}) => {
Pares.sendImageAsSticker(m.chat, data.url, m, { packname: global.packname, author: global.author })
})
break
case 'tourl':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, { sticker: gehdhe, contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
Pares.sendMessage(m.chat, { sticker: veireuue, contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
                ...
                await Pares.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' } }, { quoted: m })
                await fs.unlinkSync(media)
            }
            break
case 'tovn':{
...
if (!/video/.test(mime) && !/audio/.test(mime)) throw `Reply Video/Audio Yang Ingin Dijadikan VN Dengan Caption ${prefix + command}`
            ...
            Pares.sendMessage(m.chat, {audio: audio, mimetype:'audio/mpeg', ptt:true }, {quoted:m})
            }
            break
case 'tomp3': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
            ...
            Pares.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${Pares.user.name}.mp3`}, { quoted : m })
            }
            break
...
let encmedia = await Pares.sendImageAsSticker(m.chat, titor, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
break
case 'dadu':
case 'patrick':
...
Pares.sendMessage(m.chat, { sticker: buffer, contextInfo: {
"mentionedJid": [sender],
"externalAdReply": {
"showAdAttribution": true,
"title": `Hai Kak ${pushname}👋🏻`, 
...
Pares.sendMessage(m.chat, buttonMessage, { quoted: fakePares })
}
break
case 'quotes':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, {document: yy, mimetype: 'application/pdf', fileName: `iqra${text}.pdf`}, {quoted:fakePares}).catch ((err) => reply(oh))
}
break
...
if (args[0] === 'pdf') {
sticWait(from)
Pares.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.pdf'}, mimetype: 'application/pdf', fileName: 'juz-amma-arab-latin-indonesia.pdf'}, {quoted:fakePares})
} else if (args[0] === 'docx') {
sticWait(from)
Pares.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.docx'}, mimetype: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fileName: 'juz-amma-arab-latin-indonesia.docx'}, {quoted:fakePares})
} else if (args[0] === 'pptx') {
sticWait(from)
Pares.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.pptx'}, mimetype: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', fileName: 'juz-amma-arab-latin-indonesia.pptx'}, {quoted:fakePares})
} else if (args[0] === 'xlsx') {
sticWait(from)
Pares.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.xlsx'}, mimetype: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', fileName: 'juz-amma-arab-latin-indonesia.xlsx'}, {quoted:fakePares})
} else {
...
let message = await prepareWAMessageMedia({ image: buff }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
                    ...
                    await Pares.sendMessage(m.chat, { image: eahdw, caption: ini_txt }, { quoted: fakePares})                  
                    break
case 'pinterest':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
sticWait(from)
                    ...
                    await Pares.sendMessage(m.chat, { image: ini_buffer, caption: '*PINTEREST IMAGE*' }, { quoted: fakePares})                  
                    break
case 'pinterest2':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
                    ...
                    for (var x = 0; x <= 5; x++) {
                        ...
                        await await Pares.sendMessage(m.chat, { image: ini_buffer, caption: '*PINTEREST IMAGE*' }, { quoted: fakePares})                  
                    }
                    break
case 'character':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
sticWait(from)
                    ...
                    await Pares.sendMessage(m.chat, { image: gnejws, caption: ini_txt }, { quoted: fakePares})                  
                    break
case 'manga':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
sticWait(from)
                    ...
                    await Pares.sendMessage(m.chat, { image: cehdhw, caption: ini_txt }, { quoted: fakePares})                  
                    break
case 'kusonime':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
let message = await prepareWAMessageMedia({ image: ini_buffer }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
sticWait(from)
             ...
             let message = await prepareWAMessageMedia({ image: mvn }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
pll = `Nih Kak シ︎`
let message = await prepareWAMessageMedia({ image: imagenya }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
pll = `Nih Kak シ︎`
let message = await prepareWAMessageMedia({ image: dwruw }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
case 'lava': case 'rock': case 'bloodglas': case 'hallowen': case 'darkgold': case 'joker': case 'wicker':
 ...
 if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
             ...
             if (!q) return reply(`Example : ${prefix + command} Pares`) 
             reply(mess.wait)
             let link
             ...
             let anu = await maker.textpro(link, q)
                Pares.sendMessage(m.chat, { image: { url: anu }, caption: `*Done*` }, { quoted: m })
             }
break
case 'shadow':
case 'cup':
...
if (args.length == 0) return replyNya(`Example: ${prefix + command} Pares`)
sticWait(from)
txt1 = args[0]
txt2 = args[1]
...
sticWait(from)
                    ...
                    Pares.sendMessage(m.chat, { image: ini_buffer, caption: `*KTP MAKER*` }, { quoted: fakePares})
                    break
case 'tiktokmaker':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendButtonText(m.chat, butkie, `Selesai Mining🚧\nlist hasil :\nEmas : ${emas}🪙\nUang : $${pp}💰\nBesi : ${besi}⛓️\nBerlian : ${dm}💎`, Pares.user.name, m)
}, 9000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: '🚧 Selesai Menguli . . .🪙👷' }, { quoted: fakePares })
}, 7000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: '🚧 Menemukan Emas . . .⚒️🏔️️️' }, { quoted: fakePares })
}, 4000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: '🚧 Mulai Menambang . . .⚒️🏔️️' }, { quoted: fakePares })
}, 1500) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: mining }, {quoted: fakePares}) 
}, 0) // 1000 = 1s,
break
case 'inventori':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendButtonText(m.chat, buttonnss, `📍 𝗣𝗿𝗼𝗳𝗶𝗹𝗲 𝗣𝗹𝗮𝘆𝗲𝗿\n • Nama : ${pushname}\n • Rank : ${role}\n • Status : ${elit}\n • Xp : ${getLevelingXp(sender)}/${reqXp}\n • Level : ${getLevelingLevel(sender)}\n🎒 𝗜𝗻𝘃𝗲𝗻𝘁𝗼𝗿𝗶 :\n • Emas : ${getEmas(sender)}🪙\n • Uang : $${(getBalance(sender, balance))}💰\n • Besi : ${getBesi(sender)}⛓️\n • Berlian : ${getDm(sender)}💎\n • Ikan : ${getFish(sender)}🎣`, Pares.user.name, m)
break
case 'sellikan':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendButtonText(m.chat, buttotons, `🛒 𝗣𝗔𝗦𝗔𝗥\n • Penjual : ${pushname}\n • Pembeli : Admin\n • Harga/Ikan : 5\n • Status : Sukses\n • Sisa Ikan : ${getFish(sender)}\n • Hasil Penjualan : $${rp}`,  Pares.user.name, m)
break
case 'sellbesi':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendButtonText(m.chat, butbes, `🛒 𝗣𝗔𝗦𝗔𝗥\n • Penjual : ${pushname}\n • Pembeli : Admin\n • Harga/Besi : 10\n • Status : Sukses\n • Sisa Besi : ${getBesi(sender)}\n • Hasil Penjualan : $${rp}`, Pares.user.name, m)
break
case 'sellemas':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendButtonText(m.chat, butem, `🛒 𝗣𝗔𝗦𝗔𝗥\n • Penjual : ${pushname}\n • Pembeli : Admin\n • Harga/Emas : 25\n • Status : Sukses\n • Sisa Emas : ${getEmas(sender)}\n • Hasil Penjualan : $${rp}`, Pares.user.name, m)
break 
case 'selldiamond':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendButtonText(m.chat, butdi, `🛒 𝗣𝗔𝗦𝗔𝗥\n • Penjual : ${pushname}\n • Pembeli : Admin\n • Harga/Dm : 75\n • Status : Sukses\n • Sisa Diamond : ${getDm(sender)}\n • Hasil Penjualan : $${etoo}`, Pares.user.name, m)
break 
case 'mancing':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
var button = [
        ...
        Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: cing }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(mess.error))
}, 6000)
setTimeout( () => {
Pares.sendMessage(from, { text: 'Berhasil Mendapatkan Ikan . . .' }, { quoted: fakePares })
}, 5000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: '🎣 Menarik Kail. . .' }, { quoted: fakePares })
}, 3000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: '🎣 Mulai Memancing . . .' }, { quoted: fakePares })
}, 1500) // 1000 = 1s,
...
var button = [
        ...
        var hg = `「 DEATH 」\n\n • Tempat  ${ad}\n • MONEY : $${money}\n • EXP : ${adven}Xp`
        Pares.sendMessage(from, { caption: hg, location: { jpegThumbnail: hasm }, templateButtons: button, footer: `Rpg Game By Pares`, mentions: [sender] })
        .catch ((err) => reply(mess.error))
}, 7000)
setTimeout( () => {
Pares.sendMessage(from, { text: `Awass` }, { quoted: fakePares })
}, 5000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: `Tiba Tiba Ada ${sesuatu}` }, { quoted: fakePares })
}, 3000) // 1000 = 1s,
setTimeout( () => {
Pares.sendMessage(from, { text: `${pushname} Sedang Bertualang` }, { quoted: fakePares })
}, 1500) // 1000 = 1s,
...
Pares.sendButtonText(m.chat, butluck, `🎰 Lucky\n• Uang : $${b}\n• Emas : ${a}\n• Besi : ${c}\n• Xp : ${ez}`, Pares.user.name, m)
}
break
case 'cecan2':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
pll = `Nih Kak シ︎`
let message = await prepareWAMessageMedia({ image: { url: imgd.result.url } }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
await Pares.groupAcceptInvite(result).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
...
await Pares.sendButtonText(m.chat, buttons, `Mode Antilink`, Pares.user.name, m)
}
break
case 'antiwame':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
await Pares.sendButtonText(m.chat, buttons, `Mode Antiwame`, Pares.user.name, m)
}
break
case 'autosticker':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
await Pares.sendButtonText(m.chat, buttons, `Mode Autosticker`, Pares.user.name, m)
}
					break
...
if (args[0] === 'close'){
await Pares.groupSettingUpdate(m.chat, 'announcement').then((res) => replyNya(`Sukses Menutup Group`)).catch((err) => reply(jsonformat(err)))
} else if (args[0] === 'open'){
await Pares.groupSettingUpdate(m.chat, 'not_announcement').then((res) => replyNya(`Sukses Membuka Group`)).catch((err) => reply(jsonformat(err)))
} else {
let buttons = [
{ buttonId: 'group open', buttonText: { displayText: 'Open' }, type: 1 },
{ buttonId: 'group close', buttonText: { displayText: 'Close' }, type: 1 }
]
await Pares.sendButtonText(m.chat, buttons, `Mode Group`, Pares.user.name, m)
}
}
break
case 'welcome': {
...
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
if (args.length < 1) return replyNya(`Contoh : ${prefix + command} Pares`)
cantik = body.slice(1)
...
Pares.sendMessage(m.chat, { text: `${yn}%` }, { quoted: fakePares })
break
case 'jadian': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
if (!m.isGroup) throw mess.group
...
await Pares.sendButtonText(m.chat, buttons, jawab, Pares.user.name, m, {mentions: menst})
}
break
case 'bisakah':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
var media = await Pares.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
if (args[0] == `'panjang'`) {
...
var memeg = await Pares.updateProfilePicture(m.chat, { url: media })
fs.unlinkSync(media)
reply(`Sukses`)
}
}
...
var memeg = await Pares.updateProfilePicture(botNumber, { url: media })
fs.unlinkSync(media)
reply(`Sukses`)
}
}
...
let media = await Pares.downloadAndSaveMediaMessage(quoted)
await Pares.updateProfilePicture(m.chat, { url: media }).catch((err) => fs.unlinkSync(media))
replyNya(mess.success)
}
break
...
if (!isAdmins) throw mess.admin
if (!text) throw 'Text ?'
await Pares.groupUpdateSubject(m.chat, text).then((res) => reply(mess.success)).catch((err) => reply(jsonformat(err)))
}
break
...
if (!isAdmins) throw mess.admin
if (!text) throw 'Text ?'
await Pares.groupUpdateDescription(m.chat, text).then((res) => reply(mess.success)).catch((err) => reply(jsonformat(err)))
}
break
case 'add': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
await Pares.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'kick': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
await Pares.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'tagall': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: fakePares })
}
break
case 'hidetag': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
if (!m.isGroup) throw mess.group
if (!isAdmins) throw mess.admin
Pares.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, { quoted: fakePares })
}
break
case 'promote': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
if (!isAdmins) throw mess.admin
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Pares.groupParticipantsUpdate(m.chat, [users], 'promote').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
case 'demote': {
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
if (!isAdmins) throw mess.admin
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Pares.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
...
sticWait(from)
                    ...
                    Pares.sendMessage(m.chat, { image: fol, caption: ini_txt }, { quoted: fakePares})                  
                    get_audio = await getBuffer(ani.audio[0].link)
                    Pares.sendMessage(m.chat, {audio: get_audio, mimetype: 'audio/mpeg', fileName: `${ani.title}.mp3`}, { quoted : fakePares })
                    break
case 'play':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
Pares.sendMessage(m.chat, { audio: { url: media.dl_link }, mimetype: 'audio/mp4', ptt:true, contextInfo:{externalAdReply:{
showAdAttribution: true,
                title: `Hai Kak ${pushname}`,
                body: `© 2022`,
                thumbnail: thumb,
...
sticWait(from)
                    ...
                    await Pares.sendMessage(m.chat, { image: fott, caption: ini_txt }, { quoted: fakePares})                  
                    break
case 'tiktokaudio':
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
let message = await prepareWAMessageMedia({ video : { url: anu.nowm } }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
...
Pares.sendImage(m.chat, anu.thumb, `✇ Title : ${anu.title}
✇ Channel : ${anu.channel}
✇ Upload : ${anu.published}
✇ Views : ${anu.views}`, m)
Pares.sendMessage(m.chat, { audio: { url: anu.url }, mimetype: 'audio/mp4', ptt:true, contextInfo:{externalAdReply:{
showAdAttribution: true,
title: `Hai Kak ${pushname}`,
body: `© 2022`,
thumbnail: thumb,
...
let message = await prepareWAMessageMedia({ video : { url: anu.url } }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
videoMessage: message.videoMessage,
...
Link Download : 
${anu.url}`,
hydratedFooterText: 'Jangan Lupa Subscribe YT : Pares`',
hydratedButtons: []
}
}
}), { userJid: m.chat, quoted: m })
...
title: a.title, description: `Channel: ${a.author.name} | Durasi: ${a.duration}`, rowId: `${prefix}play ${a.url}`
    ...
    const button = {
        ...
        Pares.relayMessage(m.chat, templateList.message, { messageId: templateList.key.id });
break
case 'hentai':
case 'ahegao':
case 'ass':
...
Pares.sendMessage(m.chat, { image: { url: random }, caption: `Nih Kak` }, { quoted: fakePares })
}
break
...
Pares.sendMessage(m.chat, buttonMessaage, { quoted: {
key: {
fromMe: false,
participant: `0@s.whatsapp.net`,
remoteJid: "6281903153426-1626053991@g.us"
...
Pares.sendMessage(m.chat, { image: { url: random.male }, caption: `Foto Couple Male` }, { quoted: fakePares })
Pares.sendMessage(m.chat, { image: { url: random.female }, caption: `Fofo Couple Female` }, { quoted: fakePares })
}
break
case 'pacarserti':{
if (isBan) return replyBan('Maaf Kamu Sudah Di Ban Silahkan Chat Owner Di Bawah Untuk Membuka Nya')
...
let message = await prepareWAMessageMedia({ image: wehw }, { upload: Pares.waUploadToServer })
const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
templateMessage: {
hydratedTemplate: {
imageMessage: message.imageMessage,
...
Pares.updateBlockStatus(orgnye, 'block')
reply(`Succes ban`)
} else if (args[0] === "del") {
if (!isBane) return reply('User tidak dibanned')
Pares.updateBlockStatus(orgnye, 'unblock')
reply(`Succes delban`)
} else {
reply("Error")
}
...
const listMessage = {
text: teskd,
footer: "_Creator Pares_",
title: "     「 List Participants Blockir 」",
buttonText: "List Blockir",
mentions: await Pares.parseMention(teskd),
sections
}
Pares.sendMessage(from, listMessage, {quoted:m})
} catch {
...
if (!q) return reply(`Contoh ${command} Alfax`)
aj = await instagramstalk.instagramstalk(`${q}`)
Alfa.sendMessage(m.chat, { image: { url : aj.profile }, caption: 
`*/ Stalking Instagram \\*
...
if (!q) return reply(`Contoh ${command} Alfa`)
aj = await githubstalk.githubstalk(`${q}`)
Alfa.sendMessage(m.chat, { image: { url : aj.profile_pic }, caption: 
`*/ Stalking Github \\*
...
Pares.sendMessage(m.chat, bguttonMessaage)
}
break
case 'tambah':
