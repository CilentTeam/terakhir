...
async function startalfa() {
    const alfa = alfaConnect({
        ...
        browser: ['Alfa Full Bug','Chrome','1.0.0'],
        auth: state
    })
...
alfa.sendProduct = async(jid, img, proid, text, desc, priceam) => {
const product = generateWAMessageFromContent(jid, proto.Message.fromObject({
"productMessage": {
                    "product": {
...
alfa.relayMessage(jid, product.message, { messageId: product.key.id})
}
